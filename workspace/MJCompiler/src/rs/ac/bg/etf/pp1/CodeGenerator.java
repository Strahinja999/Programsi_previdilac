...
public class CodeGenerator extends VisitorAdaptor {
	...
	public void visit(FactorMax max) {
		Designator des = max.getDesignator();
		Obj obj = des.obj;
		int elem;
		...
		Code.put(Code.store_n);	//p0 - adresa niza
		...
		Code.put(Code.load_1);
		Code.put(Code.load_2);
		Code.putFalseJump(Code.lt, 0);	//ako je manje jednako propadamo
		int endFor = Code.pc - 2;
		
		//if(niz[i] >= max) max = niz[i]
		Code.put(Code.load_n);	//adresa
		Code.put(Code.load_1);	//index
		Code.put(Code.aload);
		//tren element je na steku
		
		Code.put(Code.load_3);
		Code.putFalseJump(Code.ge, 0);	//ako je >= propadamo
		int endIf = Code.pc - 2;
